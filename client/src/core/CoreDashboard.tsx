...
export function CoreDashboard({ config }: Props) {
  ...
  const [keys, setKeys] = useState<KeysResponse>({ items: [], stats: { sources: 0, healthySources: 0, available: 0, leased: 0, total: 0, bySource: {} } });
  const [pool, setPool] = useState<PoolSnapshot>(emptyPool);
  const [tasks, setTasks] = useState<TasksResponse>({ items: [], page: 1, limit: 100, total: 0 });
      ...
      for (const task of nextTasks.items) {
        ...
        if (previous !== task.status) {
          ...
          changes.push({
            ...
            detail: `${task.input.model} · ${task.keyID || "等待 Key"} · attempt=${task.attempts}`,
  ...
  async function action(name: string, work: () => Promise<unknown>, event: Omit<CoreEvent, "id" | "time">) {
    setBusy(name);
    try {
      await work();
      addEvent(event);
  ...
  async function createKey(input: CreateKeyInput) {
    await action("create", () => apiJson("/api/keys", { method: "POST", body: JSON.stringify(input) }), {
      type: "KEY", subject: input.id || input.name, event: "REGISTERED", detail: `concurrency=${input.concurrency} · ${input.models.join(", ")}`, level: "success",
    });
  }
  ...
  async function updateConcurrency(source: KeySource, concurrency: number) {
    await action(`update:${source.id}`, () => apiJson(`/api/keys/${encodeURIComponent(source.id)}`, {
      ...
      type: "KEY", subject: source.id, event: "POOL_REBUILT", detail: `${source.concurrency} → ${concurrency} copies`, level: "success",
    });
  }

  async function toggleKey(source: KeySource) {
    await action(`toggle:${source.id}`, () => apiJson(`/api/keys/${encodeURIComponent(source.id)}/toggle`, { method: "POST" }), {
      type: "KEY", subject: source.id, event: source.enabled ? "DISABLED" : "ENABLED", detail: "KeyPool 已按新状态重建", level: "warning",
    });
  }

  async function healthTest(source: KeySource) {
    await action(`health:${source.id}`, () => apiJson(`/api/keys/${encodeURIComponent(source.id)}/health-test`, { method: "POST" }), {
      type: "KEY", subject: source.id, event: "HEALTH_TEST", detail: `GET ${source.baseUrl}`, level: "info",
    });
  }

  async function deleteKey(source: KeySource) {
    if (!window.confirm(`确定删除 Key「${source.name}」及其全部空闲副本吗？`)) return;
    await action(`delete:${source.id}`, () => apiJson(`/api/keys/${encodeURIComponent(source.id)}`, { method: "DELETE" }), {
      type: "KEY", subject: source.id, event: "DELETED", detail: "原始 Key 与 KeyPool 副本已删除", level: "error",
    });
  }

  async function healthTestAll() {
    await action("health:all", () => apiJson("/api/keys/health-test", { method: "POST" }), {
      type: "SYSTEM", subject: "KEYPOOL", event: "HEALTH_TEST_ALL", detail: `${keys.items.filter((item) => item.enabled).length} 个不重复原始 Key`, level: "info",
    });
  }

  async function cancelTask(task: CoreTaskItem) {
    await action(`task:${task.id}`, () => apiJson(`/api/tasks/${task.id}`, { method: "DELETE" }), {
      type: "TASK", subject: task.input.imageId || task.id, event: "CANCELLED", detail: task.id, level: "warning",
    });
  }
    ...
    <main className="core-dashboard">
      <header className="hero core-hero">
        <div>
          ...
          <h1>Core Control</h1>
          <p>观察任务如何进入队列、匹配模型、租用唯一 keyID，并在终态后归还副本。</p>
        </div>
      ...
      {error && <div className="system-alert"><strong>ADMIN UI ERROR</strong><span>{error}</span></div>}

      <section className="core-overview">
        <div><span>QUEUE</span><strong>{status?.queue.waiting ?? "—"}</strong><small>WAITING</small></div>
      ...
      <section className="core-flow">
        <div className="flow-node"><span>01</span><strong>QUEUE</strong><small>{queue?.pending ?? 0} pending · {queue?.retry_wait ?? 0} retry</small></div>
        <i>→</i>
        ...
        <i>→</i>
        <div className="flow-node"><span>04</span><strong>UPSTREAM</strong><small>{config.ui.default_model}</small></div>
      </section>
      ...
      <div className="core-columns">
        ...
        <section className="core-section key-sources">
          <div className="section-heading">
            ...
            <button className="button button--ghost" disabled={!!busy || !keys.items.length} onClick={() => void healthTestAll()}>轮询健康检查</button>
          </div>
          {!keys.items.length ? <div className="core-empty">当前没有注册 Key，Core 无法派发图片任务。</div> : (
      ...
      <TaskMonitor tasks={tasks.items} total={tasks.total} busyTaskId={busy.replace("task:", "")} onCancel={cancelTask} />
      ...
      <section className="core-section core-event-log">
        ...
        <div className="core-log-table">
          <div className="core-log-row core-log-head"><span>TIME</span><span>TYPE</span><span>SUBJECT</span><span>EVENT</span><span>DETAIL</span></div>
          {events.map((event) => (
          ...
          {!events.length && <div className="core-empty core-empty--small">等待 Key 或 Task 状态变化…</div>}
        </div>
      </section>
    </main>
  );
