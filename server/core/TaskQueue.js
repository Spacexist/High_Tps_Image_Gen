...
function cloneWithoutTimer(task) {
  const { retryTimer: _retryTimer, ...safe } = task;
  return structuredClone(safe);
}

function publicTask(task) {
  const safe = cloneWithoutTimer(task);
...
export class TaskQueue extends EventEmitter {
  ...
  claimFirst(canRun) {
    for (let index = 0; index < this.pendingIds.length; index += 1) {
      ...
      // Runner 需要内部输入，因此这里只删除无法克隆的 Timer，不隐藏 _inputFile。
      return { task: cloneWithoutTimer(task), resource };
    }
    return null;
  }
  ...
  get(id) { return publicTask(this.requireTask(id)); }
